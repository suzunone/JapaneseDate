...
class PhpdbgJobRunner extends JobRunner
{
     ...
     * @param \PHPUnit\Util\PHP\Job $job
     */
    public function run($job): Result
    {
    ...
    private function runProcess(Job $job, string $temporaryFile): Result
    {
        ...
        $process = proc_open(
            is_array(is_array($this->buildCommand($job, $temporaryFile)) ? implode(' ', array_map('escapeshellarg', $this->buildCommand($job, $temporaryFile))) : $this->buildCommand($job, $temporaryFile)) ? implode(' ', array_map('escapeshellarg', is_array($this->buildCommand($job, $temporaryFile)) ? implode(' ', array_map('escapeshellarg', $this->buildCommand($job, $temporaryFile))) : $this->buildCommand($job, $temporaryFile))) : (is_array($this->buildCommand($job, $temporaryFile)) ? implode(' ', array_map('escapeshellarg', $this->buildCommand($job, $temporaryFile))) : $this->buildCommand($job, $temporaryFile)),
            [
                0 => ['pipe', 'r'],
                1 => ['pipe', 'w'],
